...
async def filter_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "f:gerar":
        ...
        # aplica filtros normais primeiro
        filtered = processor.apply_filters(items, options)

        # filtros de palavra (API)
        if options.get("palavra_exata") and options.get("palavra_proxima"):
            both = words.filter_both(filtered)
            exact_txt = words.to_txt(both["exatas"], with_price=not options.get("sem_preco"))
            close_txt = words.to_txt(both["proximas"], with_price=not options.get("sem_preco"))

            for name, txt, label in [
                ("palavras_exatas.txt", exact_txt, "exatas"),
                ("palavras_proximas.txt", close_txt, "próximas"),
            ]:
                if not txt.strip():
                    continue
                with tempfile.NamedTemporaryFile(
                ...
                await context.bot.send_document(
                    ...
                    filename=name,
                    caption=f"{len(txt.splitlines())} usernames ({label})",
                )
                os.unlink(path)
            return

        if options.get("palavra_exata"):
            filtered = words.filter_exact(filtered)
        elif options.get("palavra_proxima"):
            filtered = words.filter_close(filtered)

        txt = processor.to_txt(filtered, sem_preco=options.get("sem_preco", False))
        ...
        await context.bot.send_document(
            ...
            caption=f"{len(filtered)} usernames após filtros",
        )
        os.unlink(path)
        return
